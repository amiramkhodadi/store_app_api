...
class ProductSerializer(serializers.ModelSerializer):
    ...
    def validate(self,data):
        if len(data['name']) < 6:
            raise serializers.ValidationError("name  length  must  be  more  than  6  char")
        return data
...
class CommentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        return Comment.objects.create(product_id = self.context.get('product_pk'),**validated_data)
